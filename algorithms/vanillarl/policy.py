"""
Policy network for VanillaRL: per-PII binary actions with group-based rewards.

Same as GRPO: binary (0/1) for each PII type, but uses simple REINFORCE updates.
...
class RulePolicy(nn.Module):
    ...
    MLP policy with per-PII binary actions (same as GRPO):
      ...
      - No value head (VanillaRL doesn't use value function)
    """

"""
Neural policy for the GRPO Rule Agent (per-PII binary actions).

The policy learns domain-specific patterns (generalized regex) for each domain.
Actions are binary (0=don't share, 1=share) for each PII type independently.
No allowed masks are used in the state - the model must learn the patterns.
"""
...
class RulePolicy(nn.Module):
    """
    Simple MLP policy with:
        - Input: state = [PII-present mask, scenario one-hot]
                 (NO allowed_mask - model learns domain patterns)
        - Output:
            * Bernoulli logits for each PII (length = NUM_PII)
              Each PII gets an independent binary decision (0 or 1)
            * State-value baseline V(s)
    
    The model learns which PII types should be shared for each domain
    (restaurant vs bank) through reinforcement learning with group-based rewards.
    """
    ...
    @torch.no_grad()
    def act(self, state, deterministic: bool = True, threshold: float = 0.5, directive: str = "balanced"):
